...
class itb_mcp extends CModule
{
	...
	public function CheckVisible()
	{
		...
		throw new \Exception(
			...
			. 'Подробный разбор по шагам — страница /mcp/check.php.'
		);
	}

	/**
	 * Все ли файлы модуля доехали.
	 *
	 * ⚠️⚠️ Ровно на этом установка и споткнулась в первый раз: деплой доставил
	 * `install/index.php`, но не `lib/Orm/`. Автозагрузчик не нашёл класс в
	 * `local/`, ушёл в `bitrix/modules/` — и сообщение показало путь, куда модуль
	 * никто не клал. Понять из него «доехало не всё» невозможно.
	 *
	 * ⚠️ Проверяем ДО первой записи в базу: наполовину установленный модуль хуже
	 * неустановленного — таблицы есть, кода нет, а в списке он числится рабочим.
	 */
	public function CheckFiles()
	{
		// Список общий со страницей диагностики — см. комментарий в manifest.php.
		...
		throw new \Exception(
			...
			. implode("\n", $lost) . "\n"
			. 'Ожидаются в ' . realpath(__DIR__ . '/..') . '. Обновите файлы на сервере '
